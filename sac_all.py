...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, log_dir: str, verbose=1):
        ...
        self.save_path = os.path.join(log_dir, 'best_model_sac')
...
def train_sac():
    ...
    model = SAC('MlpPolicy', env, verbose=1, batch_size=2048, seed=1, learning_starts=500000)
    ...
    model.save('model_save/sac')

def test_sac():
    log_dir = f"model_save/best_model_sac"
    env = ENV(istest=True)
    env.render = True
    env = Monitor(env, log_dir)
