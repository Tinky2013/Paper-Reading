...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, log_dir: str, verbose=1):
        ...
        self.save_path = os.path.join(log_dir, 'best_model_ddpg')
...
def train_ddpg():
    ...
    #                clip_obs=10.)
    model = DDPG('MlpPolicy', env, verbose=1, batch_size=512, seed=1)
    callback = SaveOnBestTrainingRewardCallback(check_freq=100, log_dir=log_dir)
    model.learn(total_timesteps=int(500000), callback = callback, log_interval = 100)
    model.save('model_save/ddpg')

def test_ddpg():
    log_dir = f"model_save/best_model_ddpg"
    env = ENV(istest=True)
    env.render = True
    env = Monitor(env, log_dir)
